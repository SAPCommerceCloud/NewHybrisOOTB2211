 ...
 * BusinessProcess.
 */
public class MemberDetailsActions1 extends AbstractSimpleDecisionAction<OrderProcessModel> {
    private static final Logger LOG = Logger.getLogger(MemberDetailsActions1.class);
    ...
    public Transition executeAction(OrderProcessModel orderProcessModel) {
        ...
        if (flag) {
            LOG.info("Now we are in the MemberDetailsActions1 and the return status as OK and now we are moving to MemberDetailsActions2 stage");
            return Transition.OK;
        } else {
            LOG.info("Now we are in the MemberDetailsActions1 and the return status as NOK return as ERROR message");
